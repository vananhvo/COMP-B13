...
_main:
	...
	movl	%eax, 24(%esp)		#Copies (n1) of argv[1] @ebp-8/@esp+24 (below old %ebp)
	...
	movl	%eax, 20(%esp)		#Copies (n2) of argv[2] @ebp-12/@esp+20 (below old %ebp)
	movl	%24(%esp), %eax		#Copies (n1) to %eax 
	movl	%eax, 28(%esp)		#Copies %eax (n1) to (n3) @esp+28/@ebp-4 
	jmp	L2			#Unconditional jump to L2
L3:
	movl	28(%esp), %eax		#Copies (n3) to %eax
	...
	movl	28(%esp), %eax		#Copies (n3) to %eax
	addl	%eax, %eax		#Add %eax to %eax (n3 + n3)
	movl	%eax, 28(%esp)		#Copies the (n3 + n3) to @esp+28/@ebp-4
L2:
	movl	28(%esp), %eax		#Copies (n3) to %eax
	cmpl	20(%esp), %eax		#(n3 - n2)
	jle	L3			#Go to L3 if (n3 - n2 == 0) || (SF != OF)
